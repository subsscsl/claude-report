...
2. Runs `/insights` to regenerate `~/.claude/usage-data/report.html` — the custom `/insights` command lives at `~/.copilot/commands/insights.md` and queries the local session store (`~/.copilot/session-store.db`)
...
### LaunchAgent logs
- stdout: `~/.claude/usage-data/launchagent-report.log` (auto-rotated at 5000 lines)
